    ...
    <form method="POST" action="{{route('admin-product.update', $data->id)}}">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="nama" class="form-control @error('nama') is-invalid @enderror"
                id="exampleInputEmail1" aria-describedby="emailHelp" value="{{ old('nama', $data->nama)}}">
            @error('nama')
        ...
        <div class="mb-3">
            ...
            <textarea name="spesifikasi" class="form-control @error('spesifikasi') is-invalid @enderror"
                aria-label="With textarea" placeholder="{{ old('spesifikasi', $data->spesifikasi)}}">{{$data->spesifikasi}}</textarea>
            @error('spesifikasi')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
        ...
        <div class="mb-3">
            <select name="nama_merek" class="form-select" aria-label="Default select example">
                <option value="{{$data->nama_merek}}" selected>{{$data->nama_merek}}</option>
                <option>Tentukan Merek</option>
