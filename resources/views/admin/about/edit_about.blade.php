...
<div class="card text-center">
    <div class="card-header">
        <a href="#" class="btn btn-primary">EDIT ABOUT</a>
    </div>
</div>
<div class="col-lg-8">
    <form method="post" action="{{route('admin-about.update', $data->id)}}">
        @method('put')
        @csrf
        <div class="mb-3">
            <label for="exampleInputEmail1" class="form-label">Judul</label>
            <input type="text" name="judul" class="form-control @error('judul') is-invalid  @enderror"
                id="exampleInputEmail1" aria-describedby="emailHelp" value="{{ old('judul', $data->judul)}}">
                @error('judul')
        ...
        <div class="mb-3">
            ...
            <textarea name="desc" class="form-control @error('desc') is-invalid @enderror"
                aria-label="With textarea"  placeholder="{{ old('desc', $data->desc)}}">{{$data->desc}}</textarea>
            @error('desc')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
